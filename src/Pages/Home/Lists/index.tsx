...
const Lists: React.FC<ListsProps> = ({lists, innerWidth})=>{
    ...
    return <ul className='lists'>
        {
            lists.map(list=>(
                <li key={list._id} onClick={()=>{handleClick(list.id)}} style={{background:innerWidth&&innerWidth<700||list.id == activeItem?lighten(0.2,list.color):'white', color:innerWidth&&innerWidth?list.color:activeItem===list.id?'white':'#3E3E3E'}} className='list'>
